...
# repeat filtering removing the outliers
# 7664_dc was not detected at this stage but when noted as outlier for sequencing PC 2 

eset<-loadEset(Grp = grp, 
               path.eset = "data/rawCountEset.rds", 
               Outliers = c("7609_dc","7664_dc"))
...
# remove sequencing pcs 1, 2, 3 4, & 5 -------------
modVars<-c("GT","seqPC1","seqPC2","seqPC3","seqPC4","seqPC5")
corname<-"SeqPC12345"
dat.cor[[corname]]<-correct()

mdslist[[corname]]<-makemds.set(data = dat.cor[[corname]])
mdsCompPlot(mds.list = mdslist[c("pre",corname)],fname = "MDS_Comp-Pre-SeqPC12345.pdf")
mdsplotAll(mds.set = mdslist[[corname]],fname = "MDS_SeqPC12345_all.pdf")

PCAdatlist[[corname]]<-PCvar(dat.cor[[corname]])
plothms(pclist = PCAdatlist,
        expPCs = corname,
        fname = "hmsCorrSeqPC12345.pdf")
cormatrix(pclist = PCAdatlist,
          expPCs = corname,
          fname = "CorrPlot_CorrSeqPC12345.pdf")


# remove sequencing pcs 1, 2, 3 4, 5 & 7-------------
modVars<-c("GT","seqPC1","seqPC2","seqPC3","seqPC4","seqPC5","seqPC7")
corname<-"SeqPC123457"
dat.cor[[corname]]<-correct()

mdslist[[corname]]<-makemds.set(data = dat.cor[[corname]])
mdsCompPlot(mds.list = mdslist[c("pre",corname)],fname = "MDS_Comp-Pre-SeqPC123457.pdf")
mdsplotAll(mds.set = mdslist[[corname]],fname = "MDS_SeqPC123457_all.pdf")

PCAdatlist[[corname]]<-PCvar(dat.cor[[corname]])
plothms(pclist = PCAdatlist,
        expPCs = corname,
        fname = "hmsCorrSeqPC123457.pdf")
cormatrix(pclist = PCAdatlist,
          expPCs = corname,
          fname = "CorrPlot_CorrSeqPC123457.pdf")
...
dim(pData(eset.cor))
dim(eset.cor)
dim(dat.cor$SeqPC123457)
exprs(eset.cor)<-as.matrix(dat.cor$SeqPC123457)
