...
# repeat filtering removing outliers detected in analyses for dc1m and dc3m 
...
# remove sequencing pcs 1, 2, 3 & control for GT*time -------------
modVars<-c("GT","Time","seqPC1","seqPC2","seqPC3","GT*Time")
corname<-"SeqPC123int"
dat.cor[[corname]]<-correct(interaction = T)

mdslist[[corname]]<-makemds.set(data = dat.cor[[corname]])
mdsCompPlot(mds.list = mdslist[c("pre",corname)],
            fname = "MDS_Comp-Pre-SeqPC123int.pdf")
mdsplotAll(mds.set = mdslist[[corname]],
           fname = "MDS_SeqPC123int_all.pdf")

PCAdatlist[[corname]]<-PCvar(dat.cor[[corname]])
plothms(pclist = PCAdatlist,
        expPCs = corname,
        Conds = conds0[,c("sample","GT","Time","GTtime")],
        fname = "hmsCorrSeqPC123int.pdf")
cormatrix(pclist = PCAdatlist,
          expPCs = corname,
          fname = "CorrPlot_CorrSeqPC123int.pdf")


# remove sequencing pcs 1, 2, 3 4, 5, 7 & control for GT*time -------------
modVars<-c("GT","Time","seqPC1","seqPC2","seqPC3","seqPC4","seqPC5","seqPC7","GT*Time")
corname<-"SeqPC123457int"
dat.cor[[corname]]<-correct(interaction = T)

mdslist[[corname]]<-makemds.set(data = dat.cor[[corname]])
mdsCompPlot(mds.list = mdslist[c("pre",corname)],
            fname = "MDS_Comp-Pre-SeqPC123457int.pdf")
mdsplotAll(mds.set = mdslist[[corname]],
           fname = "MDS_SeqPC123457int_all.pdf")

PCAdatlist[[corname]]<-PCvar(dat.cor[[corname]])
plothms(pclist = PCAdatlist,
        expPCs = corname,
        Conds = conds0[,c("sample","GT","Time","GTtime")],
        fname = "hmsCorrSeqPC123457int.pdf")
cormatrix(pclist = PCAdatlist,
          expPCs = corname,
          fname = "CorrPlot_CorrSeqPC123457int.pdf")
...
dim(pData(eset.cor))
dim(eset.cor)
dim(dat.cor$SeqPC123457int)
exprs(eset.cor)<-as.matrix(dat.cor$SeqPC123457int)
