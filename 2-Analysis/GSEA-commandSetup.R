...
pathBase="path.to.where.run.files.are"

genesets=c(hallmark="h.all.v7.0.symbols.gmt",
           GOBP="c5.bp.v7.0.symbols.gmt",
           KEGG="c2.cp.kegg.v7.0.symbols.gmt")

genesetPath="path.to.GeneSets"


cmdFileDir=file.path(pathBase,"runFiles")
...
cat("\n\n")
cat("cd path.to.GSEA.executable && source  full.path/runFiles/*_${SGE_TASK_ID}.sh")
sink()
