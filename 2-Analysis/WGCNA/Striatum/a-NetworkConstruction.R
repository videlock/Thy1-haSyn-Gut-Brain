...
eset<-readRDS("../../../1-RNAseqWorkflow/C_QCandNormalization/str/data/CorrectedEset.rds") # path to corrected eset
an<-fData(eset)
...
main="Striatum signed bicor network"
sub=paste0("power=",sp,", cut height=",ch,", min. mod. size=",mms,", deep split=",ds,
           ", mod. merge threshold=",dthresh)
pdf(file = "Signed_Dendro_params_Final.pdf",height=5,width=10
);plotDendroAndColors(geneTree, mColorh, groupLabels = mLabelh, 
                      addGuide=TRUE,dendroLabels=FALSE, main= main,
                      autoColorHeight = F,
