#!/usr/bin/env Rscript
#$ -cwd
#$ -o logs/go_$JOB_ID_$TASK_ID.txt
#$ -j y
#$ -t 1:204
#  Resources requested:
...
gi<-readRDS("geneINFOboth_MM6.rds")
...
if(MMthresh=="all"){
  ...
  gi2<-gi[gi$MM.dc>=0.5&gi$MM.str>=0.5,]
  gi2<-gi2[gi2$MM.dc>=as.numeric(MMthresh)/10|gi2$MM.str>=as.numeric(MMthresh)/10,]
